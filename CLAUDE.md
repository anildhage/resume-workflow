...
- `profiles/anil/` - Source profile data (user's personal information and experience)
- `scripts/create_resume.py` - Main orchestration script

## How It Works
1. The system reads `jd.md` to understand job requirements
2. It loads profile data from `profiles/anil/` 
...
## Profile Structure
The system uses `profiles/anil/` as the source data, not `files/`. Key files:
...
Note: The `files/` directory is only for output, not source data. Source profile information comes from `profiles/anil/`.