#!/usr/bin/env python3
"""
AI-powered resume content generator that creates both .md and .pdf versions.
This script analyzes the job description and generates tailored content for each section,
following the exact sequential flow specified, and outputs both formats.
"""
...
def create_markdown_resume(profile_data, job_requirements):
    """Generate markdown resume content"""

    print("Creating Markdown resume...")

    career_summary = generate_career_summary(profile_data, job_requirements)
    skills_section = generate_skills_section(profile_data, job_requirements)
    work_experience = generate_work_experience(profile_data, job_requirements)
    projects_section = generate_projects_section(profile_data, job_requirements)
...
def create_pdf_resume(markdown_content):
    """Create PDF version from markdown (this would require pandoc or similar)"""
    ...
    try:
        ...
        # If pandoc is not available, create a note that PDF generation failed
        print("Warning: Could not generate PDF (pandoc not installed or not in PATH)")
        print("Markdown version generated successfully.")
        return False
...
def main():
    ...
    print("Creating both .md and .pdf versions...")
    ...
    markdown_content = create_markdown_resume(profile_data, job_analysis)

    # Write markdown file
    with open('generated_resume.md', 'w') as f:
        f.write(markdown_content)

    print("Markdown resume saved to generated_resume.md")

    # Create PDF version
    create_pdf_resume(markdown_content)

    print("=" * 50)
    print("Resume generation complete!")
    print("Files created:")
    print("- generated_resume.md (markdown)")
    print("- generated_resume.pdf (PDF - if pandoc was available)")
    print("=" * 50)
